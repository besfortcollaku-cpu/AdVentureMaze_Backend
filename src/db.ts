...
function getPool() {
  if (!pool) {
    ...
    if (!url) {
      throw new Error(
        "Missing DATABASE_URL env var (Render Postgres connection string)."
      );
    }
    pool = new Pool({
...
async function ensureSchema(c: PoolClient) {
  ...
  // ✅ FIX: these must be strings (backticks)
...
export async function getUserByUsername(username: string, client?: PoolClient) {
  const run = async (c: PoolClient) => {
    const r = await c.query(`SELECT * FROM users WHERE username = $1 LIMIT 1`, [
      username,
    ]);
    return (r.rows[0] || undefined) as
      | {
...
export async function getProgressByUid(uid: string) {
  ...
  const c = await p.connect();
  try {
    const r = await c.query(`SELECT * FROM progress WHERE uid = $1 LIMIT 1`, [
      uid,
    ]);
    const row = r.rows[0];
    ...
    if ((cooldownSeconds || 0) > 0) {
      ...
      if (lastAt) {
        ...
        if (diffSeconds < cooldownSeconds) {
          throw new Error(
            `Cooldown: wait ${Math.ceil(cooldownSeconds - diffSeconds)}s`
          );
        }
      }
    }
  ...
  try {
    const r = await c.query(
      ...
      [
        ...
        Math.max(1, Math.trunc(limit || 50)),
...
export async function adminGetUser(uid: string) {
  ...
  try {
    ...
    const pr = await c.query(`SELECT * FROM progress WHERE uid = $1 LIMIT 1`, [
      uid,
    ]);
...
export async function adminGetStats({
  ...
  try {
    const users = await c.query(
      `SELECT COUNT(*)::int AS count, COALESCE(SUM(coins),0)::bigint AS coins_sum FROM users`
    );
    ...
    const levels = await c.query(
      `SELECT COUNT(*)::int AS count FROM level_rewards`
    );
