...
exports.claimDailyReturnReward = claimDailyReturnReward;
exports.claimDailyReturnRewardDouble = claimDailyReturnRewardDouble;
...
function resolveAdminProgressionPreviewState(levelNumber, frontierLevel, canPlayNow) {
...
function buildAdminProgressionDebug(user, progress, levelAccess) {
    ...
    return {
        progressionDebug: {
            ...
            dailyLevelsUnlocked: Number(levelAccess?.dailyLevelsUnlocked ?? user?.daily_levels_unlocked ?? economy_1.INITIAL_DAILY_UNLOCKED_LEVELS),
            dailyLevelsMax: Number(levelAccess?.dailyLevelsMax ?? economy_1.DAILY_LEVELS_MAX),
...
function getDailyReturnRewardForStreak(streakDay) {
...
function formatDailyReturnRewardLabel(itemType, itemCount) {
    const label = itemType === "hint" ? "Hint" : itemType === "restart" ? "Restart" : "Skip";
    return `${itemCount} ${label}${itemCount === 1 ? "" : "s"}`;
}
async function claimDailyReturnReward(uid) {
    const dayKey = getDayKey();
    const client = await exports.pool.connect();
    try {
        await client.query("BEGIN");
        const userRes = await client.query(`SELECT uid,
              daily_streak,
        ...
        const updatedRes = await client.query(`UPDATE public.users
          SET ${balanceColumn} = COALESCE(${balanceColumn}, 0) + $1,
              ...
              updated_at = NOW()
        WHERE uid = $4
        RETURNING hints_balance, restarts_balance, skips_balance, daily_streak, last_daily_claim_date, daily_reward_doubled, monthly_login_days`, [reward.itemCount, reward.streakDay, dayKey, uid]);
        await client.query("COMMIT");
        await auditRewardEvent({
            uid,
...
async function claimDailyReturnRewardDouble(uid) {
    const dayKey = getDayKey();
    const client = await exports.pool.connect();
    try {
        await client.query("BEGIN");
        ...
        const updatedRes = await client.query(`UPDATE public.users
          SET ${balanceColumn} = COALESCE(${balanceColumn}, 0) + $1,
              ...
              updated_at = NOW()
        WHERE uid = $2
        RETURNING hints_balance, restarts_balance, skips_balance, daily_streak, last_daily_claim_date, daily_reward_doubled`, [reward.itemCount, uid]);
        await client.query("COMMIT");
        await auditRewardEvent({
            uid,
...
async function adminDeleteUser(uid) {
    ...
    try {
        ...
        if (await tableExists(client, "public.user_invites")) {
            await client.query(`DELETE FROM public.user_invites
          WHERE invitee_uid = $1
             OR inviter_uid = $1`, [uid]);
        }
