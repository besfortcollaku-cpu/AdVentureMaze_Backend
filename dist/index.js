...
app.get("/api/me", async (req, res) => {
    ...
    try {
        ...
        const dailyReturnRewardResult = await (0, db_1.claimDailyReturnReward)(uid).catch(() => ({ already: true }));
        ...
        const skippedLevels = await (0, db_1.getSkippedLevels)(uid).catch(() => []);
...
app.post("/api/rewards/level-complete", async (req, res) => {
    try {
        ...
        if (level > savedLevel) {
            const userLevelRes = await db_1.pool.query(`SELECT COALESCE(mc_balance, coins, 0) AS mc_balance
           FROM public.users
          WHERE uid = $1
          LIMIT 1`, [uid]);
...
app.post("/api/rewards/daily-claim", async (req, res) => {
    try {
        const { uid } = await requirePiUser(req);
        const out = await (0, db_1.claimDailyReturnReward)(uid);
        res.json({
            ok: true,
...
app.post("/api/rewards/daily-return/double", async (req, res) => {
    try {
        ...
        const out = await (0, db_1.claimDailyReturnRewardDouble)(uid);
        if (!out?.already) {
            try {
                await (0, db_1.trackRewardedAdActivity)({
                    uid,
       ...
       WHERE uid=$1`, [uid]);
        ...
        await db_1.pool.query(`UPDATE public.progress
          SET level = GREATEST(level, $2),
              updated_at = NOW()
        WHERE uid = $1`, [uid, nextAccessibleLevel]);
        await db_1.pool.query("COMMIT");
        try {
            await (0, db_1.recalcAndStoreMonthlyRate)(uid);
        }
        ...
        const updatedProgress = await db_1.pool.query(`SELECT free_skips_used, level FROM progress WHERE uid=$1`, [uid]);
        const skippedLevels = await (0, db_1.getSkippedLevels)(uid).catch(() => []);
        const levelAccess = await (0, db_1.getDailyLevelAccessState)(uid).catch(() => null);
        res.json({
            ok: true,
