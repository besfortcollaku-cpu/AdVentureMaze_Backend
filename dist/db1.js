"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.pool = void 0;
exports.initDB = initDB;
exports.upsertUser = upsertUser;
exports.getProgressByUid = getProgressByUid;
exports.setProgressByUid = setProgressByUid;
exports.claimReward = claimReward;
exports.claimDailyLogin = claimDailyLogin;
exports.claimLevelComplete = claimLevelComplete;
exports.logAdEvent = logAdEvent;
exports.getTodayCoinAdReward = getTodayCoinAdReward;
exports.consumeFreeSkip = consumeFreeSkip;
exports.consumeFreeHint = consumeFreeHint;
exports.startSession = startSession;
exports.pingSession = pingSession;
exports.endSession = endSession;
exports.touchUserOnline = touchUserOnline;
exports.adminListUsers = adminListUsers;
exports.adminGetUser = adminGetUser;
exports.adminDeleteUser = adminDeleteUser;
exports.adminGetStats = adminGetStats;
exports.adminListOnlineUsers = adminListOnlineUsers;
exports.adminResetFreeCounters = adminResetFreeCounters;
exports.adminChartCoins = adminChartCoins;
exports.adminChartActiveUsers = adminChartActiveUsers;
exports.useSkip = useSkip;
exports.useHint = useHint;
exports.claimRewardLegacy = claimRewardLegacy;
exports.claimDailyLoginLegacy = claimDailyLoginLegacy;
exports.claimLevelCompleteLegacy = claimLevelCompleteLegacy;
exports.adminAuth = adminAuth;
const pg_1 = require("pg");
/* ============================================================
   DATABASE CONNECTION
============================================================ */
exports.pool = new pg_1.Pool({
    connectionString: process.env.DATABASE_URL,
    ssl: process.env.NODE_ENV === "production"
        ? { rejectUnauthorized: false }
        : false,
});
/* ============================================================
   INIT DB (EXTENDED, NOT REPLACED)
============================================================ */
async function initDB() {
    await exports.pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      uid TEXT PRIMARY KEY,
      username TEXT,
      coins INTEGER DEFAULT 0,
      created_at TIMESTAMP DEFAULT NOW()
    );

    CREATE TABLE IF NOT EXISTS progress (
      uid TEXT PRIMARY KEY REFERENCES users(uid) ON DELETE CASCADE,
      level INTEGER DEFAULT 1,
      updated_at TIMESTAMP DEFAULT NOW()
    );

    CREATE TABLE IF NOT EXISTS sessions (
      uid TEXT,
      started_at TIMESTAMP DEFAULT NOW(),
      last_ping TIMESTAMP DEFAULT NOW()
    );

    /* -------- NEW TABLES (MERGED) -------- */

    -- Ad tracking (coins / skips / hints)
    CREATE TABLE IF NOT EXISTS ad_events (
      id SERIAL PRIMARY KEY,
      uid TEXT REFERENCES users(uid) ON DELETE CASCADE,
      type TEXT CHECK (type IN ('coin','skip','hint')) NOT NULL,
      reward_coins INTEGER DEFAULT 0,
      created_at TIMESTAMP DEFAULT NOW()
    );

    -- Monthly free counters
    CREATE TABLE IF NOT EXISTS free_counters (
      uid TEXT PRIMARY KEY REFERENCES users(uid) ON DELETE CASCADE,
      month_key TEXT NOT NULL,
      free_skips_used INTEGER DEFAULT 0,
      free_hints_used INTEGER DEFAULT 0
    );

    -- Optional ledger (future Pi conversion)
    CREATE TABLE IF NOT EXISTS coin_ledger (
      id SERIAL PRIMARY KEY,
      uid TEXT REFERENCES users(uid) ON DELETE CASCADE,
      source TEXT,
      coins INTEGER,
      created_at TIMESTAMP DEFAULT NOW()
    );
  `);
}
/* ============================================================
   USERS
============================================================ */
async function upsertUser(uid, username) {
    await exports.pool.query(`
    INSERT INTO users (uid, username)
    VALUES ($1, $2)
    ON CONFLICT (uid)
    DO UPDATE SET username = EXCLUDED.username
    `, [uid, username]);
}
async function getProgressByUid(uid) {
    const res = await exports.pool.query(`SELECT level FROM progress WHERE uid = $1`, [uid]);
    return res.rows[0] || { level: 1 };
}
async function setProgressByUid(uid, level) {
    await exports.pool.query(`
    INSERT INTO progress (uid, level)
    VALUES ($1, $2)
    ON CONFLICT (uid)
    DO UPDATE SET level = $2, updated_at = NOW()
    `, [uid, level]);
}
/* ============================================================
   COINS & REWARDS
============================================================ */
async function claimReward(uid, coins, source = "reward") {
    await exports.pool.query(`UPDATE users SET coins = coins + $1 WHERE uid = $2`, [coins, uid]);
    await exports.pool.query(`INSERT INTO coin_ledger (uid, source, coins) VALUES ($1, $2, $3)`, [uid, source, coins]);
}
async function claimDailyLogin(uid) {
    await claimReward(uid, 5, "daily_login");
}
async function claimLevelComplete(uid, level) {
    await claimReward(uid, 1, `level_${level}`);
}
/* ============================================================
   ADS (NEW LOGIC)
============================================================ */
async function logAdEvent(uid, type, rewardCoins = 0) {
    await exports.pool.query(`
    INSERT INTO ad_events (uid, type, reward_coins)
    VALUES ($1, $2, $3)
    `, [uid, type, rewardCoins]);
    if (rewardCoins > 0) {
        await claimReward(uid, rewardCoins, "ad");
    }
}
/* Coin ad decay: 50 → 49 → ... → min 2 */
async function getTodayCoinAdReward(uid) {
    const res = await exports.pool.query(`
    SELECT COUNT(*)::int AS cnt
    FROM ad_events
    WHERE uid = $1
      AND type = 'coin'
      AND created_at::date = CURRENT_DATE
    `, [uid]);
    const watched = res.rows[0]?.cnt ?? 0;
    return Math.max(50 - watched, 2);
}
/* ============================================================
   FREE SKIPS / HINTS (MONTHLY)
============================================================ */
function currentMonthKey() {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
}
async function ensureFreeRow(uid) {
    const month = currentMonthKey();
    await exports.pool.query(`
    INSERT INTO free_counters (uid, month_key)
    VALUES ($1, $2)
    ON CONFLICT (uid)
    DO UPDATE SET month_key = $2
    `, [uid, month]);
}
async function consumeFreeSkip(uid) {
    await ensureFreeRow(uid);
    await exports.pool.query(`UPDATE free_counters SET free_skips_used = free_skips_used + 1 WHERE uid = $1`, [uid]);
}
async function consumeFreeHint(uid) {
    await ensureFreeRow(uid);
    await exports.pool.query(`UPDATE free_counters SET free_hints_used = free_hints_used + 1 WHERE uid = $1`, [uid]);
}
/* ============================================================
   SESSIONS
============================================================ */
async function startSession(uid) {
    await exports.pool.query(`INSERT INTO sessions (uid) VALUES ($1)`, [uid]);
}
async function pingSession(uid) {
    await exports.pool.query(`UPDATE sessions SET last_ping = NOW() WHERE uid = $1`, [uid]);
}
async function endSession(uid) {
    await exports.pool.query(`DELETE FROM sessions WHERE uid = $1`, [uid]);
}
async function touchUserOnline(uid) {
    await pingSession(uid);
}
/* ============================================================
   ADMIN
============================================================ */
async function adminListUsers() {
    const res = await exports.pool.query(`SELECT * FROM users`);
    return res.rows;
}
async function adminGetUser(uid) {
    const res = await exports.pool.query(`SELECT * FROM users WHERE uid = $1`, [uid]);
    return res.rows[0];
}
async function adminDeleteUser(uid) {
    await exports.pool.query(`DELETE FROM users WHERE uid = $1`, [uid]);
}
async function adminGetStats() {
    const res = await exports.pool.query(`
    SELECT
      COUNT(*) AS users,
      SUM(coins) AS total_coins
    FROM users
  `);
    return res.rows[0];
}
async function adminListOnlineUsers() {
    const res = await exports.pool.query(`SELECT * FROM sessions`);
    return res.rows;
}
async function adminResetFreeCounters() {
    await exports.pool.query(`DELETE FROM free_counters`);
}
async function adminChartCoins() {
    const res = await exports.pool.query(`
    SELECT DATE(created_at) d, SUM(coins) c
    FROM coin_ledger
    GROUP BY d
    ORDER BY d
  `);
    return res.rows;
}
async function adminChartActiveUsers() {
    const res = await exports.pool.query(`
    SELECT DATE(last_ping) d, COUNT(DISTINCT uid)
    FROM sessions
    GROUP BY d
    ORDER BY d
  `);
    return res.rows;
}
/* ============================================================
   🔁 LEGACY COMPATIBILITY LAYER (DO NOT REMOVE)
============================================================ */
/**
 * OLD skip logic wrapper
 * Used by index.ts
 */
async function useSkip(uid) {
    // try free skip first
    await consumeFreeSkip(uid);
    // return user state as before
    const res = await exports.pool.query(`SELECT * FROM users WHERE uid = $1`, [uid]);
    return {
        already: false,
        user: res.rows[0],
    };
}
/**
 * OLD hint logic wrapper
 */
async function useHint(uid) {
    await consumeFreeHint(uid);
    const res = await exports.pool.query(`SELECT * FROM users WHERE uid = $1`, [uid]);
    return {
        already: false,
        user: res.rows[0],
    };
}
/**
 * OLD reward wrapper
 */
async function claimRewardLegacy(uid, coins) {
    await claimReward(uid, coins, "legacy");
    const res = await exports.pool.query(`SELECT * FROM users WHERE uid = $1`, [uid]);
    return {
        already: false,
        user: res.rows[0],
    };
}
/**
 * OLD daily login wrapper
 */
async function claimDailyLoginLegacy(uid) {
    await claimDailyLogin(uid);
    const res = await exports.pool.query(`SELECT * FROM users WHERE uid = $1`, [uid]);
    return {
        already: false,
        user: res.rows[0],
    };
}
/**
 * OLD level complete wrapper
 */
async function claimLevelCompleteLegacy(uid, level) {
    await claimLevelComplete(uid, level);
    const res = await exports.pool.query(`SELECT * FROM users WHERE uid = $1`, [uid]);
    return {
        already: false,
        user: res.rows[0],
    };
}
/* ============================================================
   🔐 ADMIN AUTH PLACEHOLDER (FIXES BUILD)
============================================================ */
function adminAuth(req, res, next) {
    // TEMP: allow all admin routes
    // Later we add real auth
    next();
}
