...
async function setProgressByUid({ uid, level, coins, }) {
    await pool.query(`
...
async function claimReward({ uid, type, nonce, amount, cooldownSeconds, }) {
    const { rowCount } = await pool.query(`SELECT 1 FROM reward_claims WHERE uid=$1 AND nonce=$2`, [uid, nonce]);
    if (rowCount)
        return { already: true };
    await pool.query(`
...
async function claimLevelComplete(uid, level) {
    const { rowCount } = await pool.query(`SELECT 1 FROM level_rewards WHERE uid=$1 AND level=$2`, [uid, level]);
    if (rowCount)
        return { already: true };
    await pool.query(`
...
async function endSession(uid) {
    const { rows } = await pool.query(`DELETE FROM sessions WHERE uid=$1 RETURNING *`, [uid]);
    return rows[0];
}
/* =====================================================
   ADMIN
...
async function adminGetUser(uid) {
    ...
    const { rows: stats } = await pool.query(`SELECT type,COUNT(*) FROM reward_claims WHERE uid=$1 GROUP BY type`, [uid]);
    const { rows: session } = await pool.query(`SELECT * FROM sessions WHERE uid=$1`, [uid]);
    return {
        user,
        progress,
        stats,
...
async function adminDeleteUser(uid) {
    ...
    // delete progress
    await pool.query(`DELETE FROM progress WHERE uid = $1`, [uid]);
    // delete sessions
...
async function getCompletedLevels(uid) {
    const { rows } = await pool.query(`SELECT level FROM level_rewards WHERE uid=$1`, [uid]);
